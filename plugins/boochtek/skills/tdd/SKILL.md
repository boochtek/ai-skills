...
## Pending Specs Workflow

When implementing a feature with multiple specs, use the **pending-specs workflow**.
Specs are organized into logical **steps** — groups of related specs that share
an implementation milestone. Each step may contain one or more specs.

### Setup Phase

1. Write all specs for the feature, each marked `pending "reason"`
2. Organize specs into steps (logical implementation milestones)
3. Verify all specs run as pending (0 failures)
4. Get user approval on the spec suite

### Per-Spec Cycle (within a step)

For each pending spec in the current step:

1. **Remove `pending`** from one spec
2. **RED**: Run the spec, verify it fails for the right reason
3. **GREEN**: Write minimal code to make it pass
4. **REFACTOR**: Improve the code while keeping the spec green
5. **WIP commit**: Commit with message format below

```
WIP: pass "spec description"

Spec: path/to/spec_file.rb
Changed: path/to/changed_file1.rb, path/to/changed_file2.rb
```

### Step Completion

Once **all specs for the current step** pass:

1. Run full test suite to catch regressions
2. Squash the step's WIP commits into one clean commit
3. Run code reviews on the squashed result
4. The final commit should have no WIP prefix
5. Proceed to the next step

### Benefits

- Each per-spec cycle is self-contained — subagents can handle one spec
- WIP commits provide rollback points if something goes wrong
- Red-green-refactor creates natural refactoring opportunities
- Step-level squashing produces clean, atomic commits

### Subagent Delegation

Each per-spec cycle can be delegated to a `coder` subagent. Always provide
the broader feature context alongside the focused task — the subagent should
understand how this spec fits into the larger implementation.

```
We are implementing [feature description]. The overall steps are:
1. [step 1 summary]
2. [step 2 summary]
...

You are working on Step N: [step summary].

In spec/path/to/spec.rb, remove the `pending` from the spec
"spec description here". Make it pass with minimal changes.
Then refactor. Run the specs to verify. Do NOT modify other pending specs.
```
